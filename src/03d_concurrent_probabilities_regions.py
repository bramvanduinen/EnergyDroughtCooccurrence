...
os.chdir("/usr/people/duinen/MSc-thesis/src")
import utils as ut
from config import CLUSTER_NAMES, PATH_CLUSTERS, PATH_DATA, PATH_ED_REGIONS, REGIONS, VERSION
...
# move balkan to last position
row_order = row_order[1:] + [row_order[0]]
...
cg.set_xticklabels(cg.get_xmajorticklabels(), fontsize=16)
...
cbar.set_label(r"P($E_{r,column}$|$E_{r,row}$)", size=20)
...
for i, wr in enumerate(wrs):
    ...
    if difference == "absolute":
        ...
        diff = 100 * ((meanprob_wr - meanprob_all) / meanprob_all)
    ...
    if np.logical_and(
        (np.nanmax(diff) > np.nanmax(BOUNDS_GREY)),
        (np.nanmin(diff) < np.nanmin(BOUNDS_GREY)),
    ):
        extend = "both"
    elif np.nanmax(diff) > np.nanmax(BOUNDS_GREY):
        extend = "max"
    elif np.nanmin(diff) < np.nanmin(BOUNDS_GREY):
        extend = "min"
    else:
        extend = "neither"
    ...
    # write all to dict, plot later in one figure
    wr_meanprobs[titles[i]] = {}
    wr_meanprobs[titles[i]]["mean"] = meanprob_wr
    wr_meanprobs[titles[i]]["std"] = std_wr
    wr_meanprobs[titles[i]]["risk_ratios"] = risk_ratio
    wr_meanprobs[titles[i]]["mean_all"] = meanprob_all
    wr_meanprobs[titles[i]]["low_snr_indices"] = low_snr_indices
    wr_meanprobs[titles[i]]["high_pval_indices"] = high_pval_indices
    ...
    if i == 0:
        ...
        ax_bar.set_xticks([0.1, 0.5, 1], [0.1, 0.5, 1], size=15, rotation=90)
    else:
        ax_bar.set_xlim(0.1, 5)
        ax_bar.set_xticks([0.2, 0.5, 1, 2, 5], [0.2, 0.5, 1, 2, 5], size=15, rotation=90)
    ax_bar.set_xlabel(" Risk ratio", size=20)
    ...
    if difference == "absolute":
        cbar.set_label("Absolute probability difference [-]", size=20)
    elif difference == "relative":
        cbar.set_label("Probability difference [%]", size=20)
    cbar.set_ticks(tick_vals)
    cbar.set_ticklabels(tick_vals, size=16)
    ...
    plt.title(f"({alphabet[i]})", loc="left", size=20)
    plt.show()

# %% SAVE DATA
FILENAME_WR_MEANPROBS = os.path.join(PATH_DATA, VERSION, f"wr_meanprobs_{difference}.pickle")
# wr_meanprobs.to_csv(FILENAME_WR_MEANPROBS, index=False)
with open(FILENAME_WR_MEANPROBS, "wb") as f:
    pickle.dump(wr_meanprobs, f)

print("Data saved to ", FILENAME_WR_MEANPROBS)
# %%
with open(FILENAME_WR_MEANPROBS, "rb") as f:
    wr_meanprobs_loaded = pickle.load(f)
