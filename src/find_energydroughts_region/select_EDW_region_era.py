...
VAR0 = "netto_demand"
NR_OF_EVENTS = 73  # once a year events
SEASONS = ["winter"]
EVENT_LENGTHS = [7]
...
def process_events(dataset, variable, event_length, num_events):
    ...
    for _ in tqdm(range(num_events)):
        ...
        timestamps.append((min_timestamp, max_timestamp, max_event.to_numpy().item()))
        ...
        for event_length in EVENT_LENGTHS:
            ...
            for region_key in REGIONS:
                ...
                for country in REGIONS[region_key]["countries"]:
                    ...
                    data_arrays_region.append(season_data_country)
                ...
                df = pd.DataFrame(timestamps, columns=["start_time", "end_time", VAR0])
