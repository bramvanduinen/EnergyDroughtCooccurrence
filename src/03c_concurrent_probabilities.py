...
# OLD ONE, THIS IS SALTONS INDEX NOT A PROBABILITY! COUNT_TO_CONDITIONAL I A PROBABILITY
def count_to_probability(co_occur_count_df, country_order):
...
if not os.path.exists(os.path.join(PATH_DATA, VERSION, "co_occurrence_dict.pickle")):
    ...
    for i, wr in enumerate(wrs):
        ...
        probs_table, base_rates = count_to_probability(counts_df_wr, np.array(COUNTRIES)[ROW_ORDER])
    ...
    # write
    with open(os.path.join(PATH_DATA, VERSION, "co_occurrence_dict.pickle"), "wb") as f:
