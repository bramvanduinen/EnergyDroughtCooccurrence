...
# Bayesian probabilistic assignment (adapted from Falkena et al., 2023)


def distance_obs_erai(data, theta):
    """Distance between regimes and data (Falkena et al., 2023)."""
...
def normal_distr(mu, cov_inv, cov_det, k_nr, x):
    """Normal distribution to get probability based on mean and covariance (Falkena et al., 2023)."""
...
def prob_obs_erai_mvn(dist_obs, mean, covinv, covdet, k_nr):
    """Probability of the observations (Falkena et al., 2023)."""
...
def get_Bayes_parameters(df_cluster, k_nr):
    """Compute the prior, transition matrix, and parameters for the Bayesian probabilistic assignment (Falkena et al., 2023)."""
...
def bayes_posterior(
    ...
    """Compute the posterior probability of each regime using Bayes theorem.
    Apply hard assignment if posterior is above certain threshold (Falkena et al., 2023).
    """
