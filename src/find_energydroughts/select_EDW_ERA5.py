...
# from config_edw import COUNTRIES, EVENTTYPE, OFOLDER, RUNNAME
...
VAR0 = "netto_demand"
NR_OF_EVENTS = 73  # once a year events; ERA5 spans from 1950-2023
SEASONS = ["winter"]
EVENT_LENGTHS = [7]
# FILENAME = "/net/pc230050/nobackup/users/duinen/ERA5_energy_output/era5_energy_output_1950_2022.nc"
...
def process_events(dataset, variable, event_length, num_events):
    ...
    for _ in tqdm(range(num_events)):
        ...
        events.append(timeslice)
        timestamps.append((min_timestamp, max_timestamp))

        # event_data = dataset[variable].sel(time=timeslice, runs=max_run).expand_dims("runs")
        event_data = dataset[variable].sel(time=timeslice)
...
def main():
    ...
    for season in SEASONS:
        for event_length in EVENT_LENGTHS:
            ...
            filename = f"{EVENTTYPE}_{VAR0}_el{event_length}_{season}_{RUNNAME}_{NR_OF_EVENTS}_events_v2.csv"
